...
*Partially performed. Preliminary receive-state Q1 pin voltages have been
recorded, but the transistor pins are too difficult to reach reliably with the
DVM probes, so these readings may not be accurate. The board input voltages,
transmit reading, and RF GAIN sweep remain to be measured.*
...
| Q1 pin 1 — Drain, receive | 11.4 V | 11.552 V | 12.27 V | Preliminary; probe contact was difficult, so value may not be accurate |
| Q1 pin 2 — Gate 2, receive | 3.9 V | 3.900 V | 2.4 V | Preliminary; probe contact was difficult, so value may not be accurate |
| Q1 pin 3 — Gate 1, receive | 1.1 V | 1.051 V | 1.534 V | Preliminary; probe contact was difficult, so value may not be accurate |
| Q1 pin 4 — Source, receive | 1.6 V | 2.071 V | 1.53 V | Preliminary; probe contact was difficult, so value may not be accurate |
| Implied V_GS, receive | −0.5 V | −1.02 V | +0.004 V | Preliminary calculation from uncertain pins 3 and 4; not yet reliable |
| Implied drain current | 3.40 mA | 4.41 mA | 3.26 mA | Preliminary calculation from uncertain pin 4; strictly the source current |
| Q1 pin 1 — Drain, transmit | 0.05 V | ≈0 V | | |
| `R` pin, RF GAIN full CCW | — | 0 V assumed; see Simulation 5 | | |

If confirmed, the source voltage and implied current would be close to the
manual's example and substantially below the empirical model's result. The
preliminary gate-1 and source readings imply only +4 mV of gate-to-source bias
rather than the manual's −0.5 V, but the difficult probe access makes that small
difference unreliable. These readings should not be used to re-scale the gain
model until they can be repeated with secure test connections; record `R` and
`DEFEAT` in the same radio state when repeating them.
...
#### Hardware validation withdrawn — direct gain at the pins

The proposed two-channel oscilloscope check was tried and withdrawn. At a drive
low enough to keep the receiver out of its strong-signal/AGC region, the ×10
probe and oscilloscope could not measure the RF-amplifier output reliably. Raising
the drive until both pins were visible produced a `+30 dB` S-meter indication,
which is not a trustworthy small-signal gain condition. The apparent gain from
that trial is therefore **invalid and is not retained as a measurement**.

The Ballantine audio voltmeter could find a repeatable RESONATE peak at low drive,
but that observation does not separate the RF amplifier's gain from the mixer,
IF, detector, AGC and audio stages. No replacement pin-to-pin bench check is
specified until a sensitive detector with acceptably low loading at `OUT` has
been established.
...
| Level | 10 mVpp; halve the drive and confirm that measured gain changes by no more than 0.5 dB |
...
Six remaining bench checks are tracked, and they are not equally urgent:
...
- **Real coil inductance versus rack position.** The fitted `L_RACK` parameter has
  no established relationship to a RESONATE control position. The control is
  continuous and unmarked, so an ordinary front-panel reading cannot establish a
  reproducible position; doing so would require adding a temporary external position scale or
  measuring the rack displacement directly.
