...
the board pins that would corroborate it, and an empty table for the result.
Nothing in this section is yet corroborated by hardware.
...
| Q1 pin 1 — Drain, receive | 11.4 V | 11.552 V | | |
| Q1 pin 2 — Gate 2, receive | 3.9 V | 3.900 V | | |
| Q1 pin 3 — Gate 1, receive | 1.1 V | 1.051 V | | |
| Q1 pin 4 — Source, receive | 1.6 V | 2.071 V | | |
| Implied V_GS, receive | −0.5 V | −1.02 V | | |
| Implied drain current | 3.40 mA | 4.41 mA | | |
| Q1 pin 1 — Drain, transmit | 0.05 V | ≈0 V | | |
| `R` pin, RF GAIN full CCW | — | 0 V assumed; see Simulation 5 | | |

A measured source voltage would settle whether the empirical 40823 model or the
manual's example device is closer to this particular radio, and would let the gain
figures below be re-scaled with confidence.
...
**No sweeps have been captured yet, so nothing in this document is corroborated by
measurement.** The bench-check tables in each section are the places those results
belong when they exist.

Seven bench checks are proposed, and they are not equally urgent:
